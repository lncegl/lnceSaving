...
export default function Insights({
  ...
  const bestMonth   = [...monthlyData].sort((a, b) => (b.deposits - b.withdrawals) - (a.deposits - a.withdrawals))[0];

  return (
    <div className="space-y-6">

      {/* ── Totals row — moved from Dashboard ── */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          {
            icon: ArrowUpCircle,
            label: 'Total deposited',
            value: fmt(totalDeposited, currencySymbol),
            iconBg: 'bg-green-100 text-green-700',
          },
          {
            icon: ArrowDownCircle,
            label: 'Total withdrawn',
            value: fmt(totalWithdrawn, currencySymbol),
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          },
          {
            ...
            sub:   bestMonth ? '+' + fmtShort(bestMonth.deposits - bestMonth.withdrawals, currencySymbol) + ' net' : 'no data yet',
            color: 'text-[#1F3D2B]',
            bg:    'bg-[#F0F7EC]',
          },
      ...
      {/* ── Balance over time ── */}
      <Section title="Balance over time" icon={TrendingUp}
        ...
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={balanceSeries} margin={{ top: 4, right: 12, bottom: 0, left: 8 }}>
            ...
            <XAxis dataKey="date" tickFormatter={labelDate}
              tick={{ fontSize: 11, fill: '#8A9E85' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
            <YAxis tick={{ fontSize: 11, fill: '#8A9E85' }} tickLine={false} axisLine={false}
              width={58} tickFormatter={(v) => fmtShort(v, currencySymbol)} />
            <Tooltip content={<ChartTooltip symbol={currencySymbol} labelFn={labelDate} />} />
            <Line type="monotone" dataKey="balance" name="Balance" stroke="url(#lineGrad)" strokeWidth={2.5}
      ...
      <Section title="Monthly deposits vs. withdrawals" icon={BarChart3}
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={monthlyData} margin={{ top: 4, right: 12, bottom: 0, left: 8 }} barCategoryGap="30%">
            ...
            <XAxis dataKey="month" tickFormatter={labelMonth}
              tick={{ fontSize: 11, fill: '#8A9E85' }} tickLine={false} axisLine={false} />
            <YAxis tick={{ fontSize: 11, fill: '#8A9E85' }} tickLine={false} axisLine={false}
              width={58} tickFormatter={(v) => fmtShort(v, currencySymbol)} />
            <Tooltip content={<ChartTooltip symbol={currencySymbol} labelFn={labelMonth} />} />
            <Legend wrapperStyle={{ fontSize: 12, paddingTop: 12 }}
              formatter={(value) => <span className="capitalize text-gray-500 font-semibold">{value}</span>} />
                ...
                return (
                  <div key={m.month} className="flex items-center gap-3 text-xs">
                    ...
                    <div className="flex-1 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                      <div className={`h-full rounded-full ${up ? 'bg-green-400' : 'bg-yellow-400'}`}
                        style={{ width: Math.min(100, Math.abs(net) / Math.max(...monthlyData.map((x) => Math.max(x.deposits, x.withdrawals, 1))) * 100) + '%' }} />
                    </div>
      ...
      {/* ── Category breakdown ── */}
      <Section title="Spending by category" icon={PieIcon}
          ...
          <ResponsiveContainer width={220} height={220} className="shrink-0">
            <PieChart>
              <Pie data={categoryBreakdown} dataKey="value" nameKey="name"
                cx="50%" cy="50%" innerRadius={52} outerRadius={86} paddingAngle={2} labelLine={false}>
                {categoryBreakdown.map((entry, i) => (
                  <Cell key={entry.name} fill={PIE_COLORS[i % PIE_COLORS.length]} />
                ))}
              </Pie>
              <Tooltip formatter={(v, name) => [fmt(v, currencySymbol), name]}
                contentStyle={{ borderRadius: 12, border: '1px solid #e5e7eb', fontSize: 12 }} />
            </PieChart>
          </ResponsiveContainer>
          <div className="flex-1 space-y-2 w-full">
            {[...categoryBreakdown].sort((a, b) => b.value - a.value).map((entry, i) => {
              ...
              return (
                <div key={entry.name} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs font-mono font-bold text-gray-700 w-20 text-right">{fmt(entry.value, currencySymbol)}</span>
                </div>
              );
            })}
          </div>
